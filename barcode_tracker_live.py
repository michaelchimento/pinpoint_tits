...
def decode(working_dir, working_file):
...
##  # set up file for saving data
    create_csv(working_dir, working_file)
    ...
    video_out_fps = 30.0
    video_out_size = (frame_width, frame_height) # (width, height)
    video_out_color = True
    fourcc = 0x7634706d
    
    #fourcc = 0x00000021 #cv2.VideoWriter_fourcc('A','V','C','1') # set codec
    ...
    frame = 0
    process_every_nth_frame = 6
    
    cv2.namedWindow("preview")
    
    while cap.isOpened():  
        timeofframe = ('{:%Y-%m-%d_%H%M%S}'.format(dt.datetime.now()))
        ret, image = cap.read()
        ...
        if ret:
            ...
            if frame%process_every_nth_frame == 0:
                ...
                if num_detections > 0:
                    write_csv(detected_tags, working_dir, working_file)
            
                cv2.imshow("preview",image)
            out.write(image)
             #this is for displaying the tracking
